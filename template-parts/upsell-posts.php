...
<?php if ( $selected_posts ) : ?>
	...
	<div class="block block-latest block-selected">
        ...
        // Featured image.
				...
				<div class="entry">
					<div class="entry-featured-image" style="background-image:url('<?php if ( has_post_thumbnail() ) : echo wp_get_attachment_image_src( get_post_thumbnail_id( $selected_post->ID ), 'large' )[0]; else : echo khonsu_get_random_image_url(); endif; ?>');"><a href="<?php echo get_the_permalink(); ?>" class="absolute-link"><span class="screen-reader-text"><?php _e( 'Linkki artikkeliin', 'khonsu' ); ?> "<?php echo get_the_title( $selected_post->ID ); ?>"</span></a></div>
